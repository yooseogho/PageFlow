...
import com.example.demo.dto.member.MemberDto.Profile;
...
//		1-4 로그인 처리(로그인 페이지를 보여줍니다.)
	...
	/*
	 * 수정해야함 누적 금액이 아니라 보유하고 있는 포인트를 알수 있게 수정해야함.10-27 유석호 해야 할 것
	 */
	// 1-7 회원 포인트 확인하는 페이지
	@Secured("ROLE_USER")
	...
	public String member_grade_page(@RequestParam(value = "memberId", required = false) String memberId,
	                                @RequestParam(value = "purchaseTotal", required = false) Long purchaseTotal, 
	                                Principal principal,
	                                Model model) {
	        ...
	        if (principal != null) { 
	         ...
	         // 최근 3개월간의 구매 총액 계산
	           ...
	           switch (memberGrade.toLowerCase()) { 	// 소문자로 변환하여 등급 비교
	            ...
	            int additionalPurchaseRequired = gradeService.calculatePurchaseRequired(purchaseTotalForLastThreeMonths.intValue(), currentGradeCode);
...
//8. 프로필 사진 변경
	...
	public String changeProfile(@RequestParam(required = false) MultipartFile newProfile,
...
//    루트 페이지로 이동시킴
	...
	public ResponseEntity<?> checkPassword(@RequestParam String password,
			@RequestParam(required = false) String referrer, Principal principal, HttpSession session) {
		if (password.isEmpty()) {
