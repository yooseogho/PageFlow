...
public class BookService {
  ...
  public List<BookDto.AuthorOtherBooks> findAuthorOtherBooks(String authorName) {
    ...
    for (Book book : books) {
      ...
      BookDto.AuthorOtherBooks CategoryParentList = new BookDto.AuthorOtherBooks(book.getBno(), book.getBookTitle(),
          book.getBookImage(), book.getAuthorName());
  ...
  /**
   * pageno, 개수 -> prev, start, end, next, pageno, 제품들을 출력한다 5. 도서 종류별 페이징 리스트 페이지
   * (도서 페이지의 두번째 부분) (중분류)
   */
  public CategoryParentPage categoryParentList(Long pageno, String memberId, Long gradeCode, Long categoryParent) {
    ...
    // 상품 목록 리스트 ex) startRownum=1 , endRownum=10 이라고 가정하면 bno가 1 부터 10까지의 책 목록 나열
    List<BookDto.BookList> books = bookDao.findBookByCategoryParent(categoryParent, memberId, gradeCode, startRownum,
        endRownum);
  ...
  /**
   * pageno, 개수 -> prev, start, end, next, pageno, 제품들을 출력한다 6. 도서 장르별 페이징 리스트 페이지
   * (도서 페이지의 마지막 부분) (소분류)
   */
  public CategoryCodePage categoryCodeList(Long pageno, String memberId, Long gradeCode, Long categoryCode,
      Long categoryParent) {
