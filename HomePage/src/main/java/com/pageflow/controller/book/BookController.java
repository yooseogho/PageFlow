...
public class BookController {
	...
	public String buyNow(@PathVariable Long bno, HttpServletRequest request, Long gradeCode,
			@RequestParam(required = false, defaultValue = "1") Long cartCount) {
	...
	public ModelAndView welcome() {
		List<BookDto.NewBook> newBook = bookService.newBook();
		List<OrderDetailsDto.ManyOrder> manyOrder = orderDetailsService.manyOrder();
		return new ModelAndView("index").addObject("newBook", newBook).addObject("manyOrder", manyOrder);
	}
